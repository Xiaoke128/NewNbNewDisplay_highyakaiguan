...
uint8_t ComunHead = 0;
uint8_t ComunTail = 0;
uint8_t CommunCount = 0;
uint8_t CommunBuff[HOST_BUFFER_SIZE] = {0};
...
static void UartHostInit(void)
{
    ...
    usart_baudrate_set(HOST_UART, StoreConf.BaudRateModbus);//115200波特率
...
{
		usart_interrupt_disable(HOST_UART, USART_INT_TC);
		usart_interrupt_disable(HOST_UART, USART_INT_RBNE);
		usart_disable(HOST_UART);
		UartHostInit();
}
...
{
    usart_data_transmit(HOST_UART, (uint8_t)ch);
...
void CommunUartSendBuf(uint8_t *buf, uint16_t len)
{
	uint16_t i = 0;
	for(i = 0; i < len; i++)
	{
